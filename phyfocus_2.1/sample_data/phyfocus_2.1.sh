...
Syntax: ./${0##*/} [-h] [-T] [-X] [-q ./file] [-f ./directory/] [-c ./file] [-H ./file] [-t num] [-e value] [-m string] [-b num] [-a num]
...
PhyFocus requires four user-provided datasets:
...
1) A query fasta file containing target, related, and root proteins (Step 1).
   ...
   - Roots represent a distant outgroup to target & related gene families; used to root the initial phylogeny.
...
cd ./sample_data
clean
./phyfocus_V2.1_draft.sh -q ./query_file.fasta -f fasta_proteins/ -H hmmr_ali.fasta -c tree_tips.csv -m LG -b 1000 -a 1000
}
...
then
    ...
    echo "Removed $diff files. phyfocus directory now contains $clean_num files. 9 files (4 from user, 5 included scripts) are required to run PhyFocus."
fi
}
...
c_error="Option error; path must be to an existing file with data. Format: -c <./csv_table>"
...
while getopts ":hq:f:c:H:t:e:m:b:a:TX" option; do
    case $option in
        h) help; exit 0;;
...
# Conduct same line fix for user query file
...
do
    ...
    cat ./formatted_query.fa ${fasta}*cdhit > ${fasta%.}_bts_ancrs 2>> error_log.txt
...
mkdir tree_editor_out; mv ./*_tips.txt ./tree_editor_out; cd ../../
}
...
do
    ...
    if [[ ! (-s ./${genus}_tipseqs.fa) ]]; then echo "Error extracting fasta seq for $file to make ${genus}_tipseqs.fa. See error log and ./align/IQ_out/tree_editor_out." >> ../../../error_log.txt; exit 1; fi
done
...
mafft --retree 2 --maxiterate 0 hmmr_significant_hits.fa > hmmr_significant_hits_ali.fa # 2>> ../../error_log.txt | tee -a ../../out_log.txt DOESNT WORK; only puts out in error log and overwrites
...
#- Similar to alignment_editor, phyfocus should have a stats output. Things like changes in # of sequences between cd-hit, hmmr, etc. should be reported.
