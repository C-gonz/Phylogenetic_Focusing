...
3) A fasta file containing the rooting proteins to root the per-species unfocused trees (Steps 2 & 3).
    - There is a minimum of 2 for roots.

4) A Tab Seperated Values (.tsv) file for phylogeny focusing (Step 3).
- There should be no column or row headers in the table.
- Column 1 gives FASTA header names (no ">") for AT LEAST 2 root proteins from the roots file.
- Column 2 gives FASTA header names (no ">") for AT LEAST 1 target AND 1 anchor protein from the query file.
...
then
    ...
    rm ./out_log.txt ./error_log.txt ./header_translation_table.tsv temp.txt all_fixed_numerical_headers.txt all_ncbi_headers.txt formatted_query.fa formatted_roots.fa 2> /dev/null
    clean_num=$(ls | wc -l)
...
echo "modifying blastout files to include hit descriptions..." | tee -a out_log.txt
cd ./blastout_maxseqs
for blastout in ./*
do
    echo "Editing" $blastout | tee -a ../out_log.txt
    names=$(cut -f 2 $blastout)
    for header in $names; do grep $header ../header_translation_table.tsv >> temp; done
    cut -f 2 temp > temp2
...
do
    ...
    if [[ ! (-s $alignment.treefile) ]]; then echo "Error making ML Iq_tree for $alignment. Check alignment in ./align" >> error_log.txt; exit 1; fi
done
...
rename '.faa_FIX_hits.fa_ali.fa ' '' ./*.treefile
...
mkdir tree_editor_out; mv ./*_tips.txt ./tree_editor_out; mv ./*_focused.treefile ./tree_editor_out; mv Rplots.pdf ./tree_editor_out; cd ../../
}
...
then
    ...
    step1_blast
    # error checkpoint
    if [[ $(ls ./fixed_fastas/ | wc -l) == $(ls ./blastout_maxseqs/ | wc -l) ]]; then echo "Step 1 BLASTs complete" | tee -a out_log.txt; else echo "Step 1 BLAST of peptide FASTAS incomplete (BLAST outputs not made for every species given)." >> error_log.txt; exit 1; fi
    echo "PhyFocus elapsed time:" $((($SECONDS/60))) "minutes ($((($SECONDS/60)/60)) hours)" | tee -a out_log.txt; echo " " | tee -a out_log.txt; echo " " | tee -a out_log.txt
else
...
then
    ...
    step2_initial_alignments
    # error checkpoint
    if [[ $(ls ./fixed_fastas | wc -l) == $(ls ./align | wc -l) ]]; then echo "Step 2 species' peptide alignments complete" | tee -a ./out_log.txt; else echo "Step 2 species' peptide alignments incomplete (alignments not made for every species)." >> ./error_log.txt; exit 1; fi
    echo "PhyFocus elapsed time:" $((($SECONDS/60))) "minutes ($((($SECONDS/60)/60)) hours)" | tee -a out_log.txt; echo " " | tee -a out_log.txt; echo " " | tee -a out_log.txt
else
...
then
    ...
    step2_initial_trees
    # error checkpoint
    if [[ $(ls ./fixed_fastas | wc -l) == $(ls ./align/IQ_out | wc -l) ]]; then echo "Step 2 species' IQTree runs are complete" | tee -a ./out_log.txt; else echo "Step 2 species' IQTree runs failed to run properly (not all treefiles for each species exist); check ./align alignments and ensure IQTree is running." >> ./error_log.txt; exit 1; fi
    echo "PhyFocus elapsed time:" $((($SECONDS/60))) "minutes ($((($SECONDS/60)/60)) hours)" | tee -a out_log.txt; echo " " | tee -a out_log.txt; echo " " | tee -a out_log.txt
else
...
then
    ...
    step3_focus
    # error checkpoint
    if [[ $(ls ./fixed_fastas | wc -l) == $(ls ./align/IQ_out/tree_editor_out/*_tips.txt | wc -l) ]]; then echo "Step 3 tree_editor.R focusing complete" | tee -a out_log.txt; else echo "Error occured when generating output tips.txt files from Step 3's tree_editor.R focusing; check error log for any R-specific messages and ensure ./align/IQ_out files are present for each species." >> error_log.txt; exit 1; fi
    echo "PhyFocus elapsed time:" $((($SECONDS/60))) "minutes ($((($SECONDS/60)/60)) hours)" | tee -a out_log.txt; echo " " | tee -a out_log.txt; echo " " | tee -a out_log.txt
else
...
then
    ...
    step4_extract_fastas
    # error checkpoint
    if [[ $(ls ./fixed_fastas | wc -l) == $(ls ./tip_seqs/* | wc -l) ]]; then echo "Step 4 extracting focused seqs complete." | tee -a out_log.txt; else echo "Step 4 extracting focused seqs incomplete (fasta for each taxa not present in ./tip_seqs); check tips.txt files in ./align/IQ_out/tree_editor_out." >> error_log.txt; exit 1; fi
    echo "PhyFocus elapsed time:" $((($SECONDS/60))) "minutes ($((($SECONDS/60)/60)) hours)" | tee -a out_log.txt; echo " " | tee -a out_log.txt; echo " " | tee -a out_log.txt
else
...
then
    ...
    step5_filtering
    # error checkpoint
    if [[ -s ./final_tree_dataset/filtering_output/hmmr_significant_hits.fa ]]; then echo "Step 5 HMMR filtering of focused seqs complete" | tee -a out_log.txt; else echo "Step 5 HMMR filtering of focused seqs incomplete (final_tree_seqs.fa not made or empty); check if ./tip_seqs is not empty and ./align outputs look correct." >> error_log.txt; exit 1; fi
    echo "PhyFocus elapsed time:" $((($SECONDS/60))) "minutes ($((($SECONDS/60)/60)) hours)" | tee -a out_log.txt; echo " " | tee -a out_log.txt; echo " " | tee -a out_log.txt
else
...
then
    ...
    step6_final_align
    # error checkpoint
    if [[ -s ./final_tree_dataset/filtering_output/final_tree_seqs_ali.fa ]]; then echo "Step 6 alignment of final tree seqs complete" | tee -a out_log.txt; else echo "Step 6 alignment of final tree seqs incomplete (final_tree_seqs_ali.fa not made or empty); check ./final_tree_dataset/filtering_output HMMR files." >> error_log.txt; exit 1; fi
    echo "PhyFocus elapsed time:" $((($SECONDS/60))) "minutes ($((($SECONDS/60)/60)) hours)" | tee -a out_log.txt; echo " " | tee -a out_log.txt; echo " " | tee -a out_log.txt
else
    echo "Restarted Phyfocus run: skipped Step 6 alignment of final tree seqs" | tee -a out_log.txt
fi

# Final outlog statements before user alignment editing.
echo "Phyfocus alignment is ready for Step 6 user assessment with alignment_editor.py. Please examine 'final_tree_dataset/filtering_output/final_tree_seqs_ali.fa' and identify problematic gaps. See alignment_editor.py -h for details." | tee -a ./out_log.txt; echo | tee -a ./out_log.txt

echo "Once alignment editing is complete, runnning IQTree on the realigned output will complete Step 6 of the phyfocus run. Recommended approach: iqtree -s final_tree_seqs_ali.fa_edited_ali.fa -m MFP+C60 -alrt 1000 -bb 1000 -nt 24" | tee -a ./out_log.txt
